...
const validationSchema = Yup.object().shape({
  ...
  testing: Yup.array()
    .max(2, "*No more than two testing options can be selected*")
    .required("*Testing Selection is required*"),
...
class ReportFormPage extends Component {
  handleReportData(data) {
    const { testing } = data;
    // const newData = Object.keys(testingData)
    //   .filter((key) => testing.includes(key))
    //   .reduce((obj, key) => {
    //     return {
    //       ...obj,
    //       [key]: true,
    ...
    const newData = Object.fromEntries(
      Object.entries(testingData).map((pair) =>
        testing.includes(pair[0]) ? [pair[0], true] : pair
      )
    );
    console.log(newData);
  }
  render() {
    const { reporter_type } = this.props.history.location.state;
    return (
      <div>
        <h2>Report</h2>
        <h4>You can only file one report in a 24 hour period.</h4>
        <Formik
          initialValues={{
            reporter_type,
            facility_type: "",
          ...
          {({ values, isSumbitting, touched, errors, handleChange }) => (
            <Form className="form_container">
              ...
              <MyCheckbox
                ...
                label="NO SHORTAGES"
              />
              <h4>My current status re COVID lab tests</h4>
              {[
